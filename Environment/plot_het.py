...
class Plots():
    def __init__(self, xs, ys, X_test, grid, grid_min, grid_or, stage='exploration'):
        self.xs = xs
        self.ys = ys
    ...
    def bench_plot(self, bench_function):
        plot = np.zeros([self.xs, self.ys])
        for i in range(len(self.X_test)):
            plot[self.X_test[i][0], self.X_test[i][1]] = bench_function[i]
    ...
    def zoom_action_zone(self, centers, dict_limits, mu, sigma, final_mu, final_sigma, bench_function):
        rows = centers
        cols = 2
